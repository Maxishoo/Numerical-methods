...
def P(x, X, n):
    ...
    for order in range(1, n):
        ...
        # умножаем на (x - x₀)(x - x₁)...(x - x_{order-1})
...
def solution(X):
    ...
    eps = M(0.1, 1.3, len(X)+1) / \
        math.factorial(len(X)+1) * abs(omega(X_star, X))
    print(
        f"Погрешность интерполяции по формуле: {eps}, реальная {P(X_star,X,len(X))}")
